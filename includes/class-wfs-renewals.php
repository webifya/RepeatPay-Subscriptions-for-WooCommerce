...
class WFS_Renewals {
	...
	public static function create_renewal_order( $subscription_id ) {
		...
		$order->update_status( 'pending', __( 'Subscription renewal awaiting customer payment.', 'subscribely-recurring-billing-for-woocommerce' ) );
		$order->save();
	...
	public static function retry_payment( $subscription_id ) {
		...
		if ( $count >= $max ) {
			...
			$order->add_order_note( __( 'Final subscription renewal reminder sent; subscription placed on hold.', 'subscribely-recurring-billing-for-woocommerce' ) );
		} else {
			WFS_Subscription::set_status( $subscription_id, 'past-due', 'payment-retry' );
			$order->add_order_note(
				sprintf(
					/* translators: 1: attempt number, 2: maximum attempts. */
					__( 'Subscription renewal reminder %1$d of %2$d sent.', 'subscribely-recurring-billing-for-woocommerce' ),
					$count,
					$max
				)
			);
