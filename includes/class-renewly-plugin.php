...
final class RENEWLY_Plugin {
	/**
	 * Singleton.
	 *
	 * @var RENEWLY_Plugin|null
	 */
	...
	private function __construct() {
		require_once RENEWLY_PLUGIN_PATH . 'includes/class-renewly-product.php';
		require_once RENEWLY_PLUGIN_PATH . 'includes/class-renewly-subscription.php';
		require_once RENEWLY_PLUGIN_PATH . 'includes/class-renewly-renewals.php';
		require_once RENEWLY_PLUGIN_PATH . 'includes/class-renewly-account.php';
		require_once RENEWLY_PLUGIN_PATH . 'includes/class-renewly-settings.php';
		require_once RENEWLY_PLUGIN_PATH . 'includes/class-renewly-site-profile.php';
		require_once RENEWLY_PLUGIN_PATH . 'includes/class-renewly-upgrade.php';
		require_once RENEWLY_PLUGIN_PATH . 'includes/class-renewly-review.php';

		RENEWLY_Product::init();
		RENEWLY_Subscription::init();
		RENEWLY_Renewals::init();
		RENEWLY_Account::init();
		RENEWLY_Settings::init();
		RENEWLY_Site_Profile::init();
		RENEWLY_Upgrade::init();
		RENEWLY_Review::init();
	...
	public static function payment_gateway_title( $title, $gateway_id ) {
		...
		$known = array(
			'stripe'                   => __( 'Credit or debit card', 'renewly-subscriptions-for-woocommerce' ),
			'stripe_cc'                => __( 'Credit or debit card', 'renewly-subscriptions-for-woocommerce' ),
			'stripe_googlepay'         => __( 'Google Pay', 'renewly-subscriptions-for-woocommerce' ),
			'stripe_applepay'          => __( 'Apple Pay', 'renewly-subscriptions-for-woocommerce' ),
			'square_credit_card'       => __( 'Credit or debit card (Square)', 'renewly-subscriptions-for-woocommerce' ),
			'ppcp-gateway'             => __( 'PayPal', 'renewly-subscriptions-for-woocommerce' ),
			'ppcp-credit-card-gateway' => __( 'Credit or debit card (PayPal)', 'renewly-subscriptions-for-woocommerce' ),
			'ppcp-card-button-gateway' => __( 'Credit or debit card (PayPal)', 'renewly-subscriptions-for-woocommerce' ),
		);
		...
		return $fallback ?: __( 'Payment method', 'renewly-subscriptions-for-woocommerce' );
	}
	...
	public static function checkout_styles() {
		...
		wp_register_style( 'renewly-checkout-compatibility', false, array(), RENEWLY_VERSION );
		wp_enqueue_style( 'renewly-checkout-compatibility' );
		wp_add_inline_style(
			'renewly-checkout-compatibility',
	...
	public static function activate() {
		require_once RENEWLY_PLUGIN_PATH . 'includes/class-renewly-subscription.php';
		RENEWLY_Subscription::register_post_type();
		flush_rewrite_rules();
	}
	...
	public static function woocommerce_notice() {
		...
		echo esc_html__( 'Renewly Subscriptions for WooCommerce requires WooCommerce to be installed and active.', 'renewly-subscriptions-for-woocommerce' );
		echo '</p></div>';
	}
}
