...
class REPEATPAY_Site_Profile {
	const CRON     = 'repeatpay_weekly_site_profile';
	...
	public static function init() {
		...
		add_action( 'add_option_' . REPEATPAY_Settings::OPTION, array( __CLASS__, 'settings_added' ), 10, 2 );
		add_action( 'update_option_' . REPEATPAY_Settings::OPTION, array( __CLASS__, 'settings_updated' ), 10, 2 );
	...
	public static function cron_schedules( $schedules ) {
		if ( ! isset( $schedules['weekly'] ) ) {
			$schedules['weekly'] = array(
				...
				'display'  => __( 'Once Weekly', 'repeatpay-subscriptions-for-woocommerce' ),
			);
		}
		return $schedules;
	}
	...
	public static function schedule() {
		if ( REPEATPAY_Settings::get( 'share_site_profile' ) ) {
	...
	public static function plugin_updated( $upgrader, $options ) {
		...
		if ( 'plugin' === ( $options['type'] ?? '' ) && in_array( plugin_basename( REPEATPAY_PLUGIN_FILE ), $plugins, true ) && REPEATPAY_Settings::get( 'share_site_profile' ) ) {
			self::send( true );
		}
	}
	...
	public static function send( $consent = null ) {
		$consent = null === $consent ? (bool) REPEATPAY_Settings::get( 'share_site_profile' ) : (bool) $consent;
		...
		if ( wp_remote_retrieve_response_code( $response ) >= 400 ) {
			$message = is_array( $data ) && isset( $data['message'] ) ? $data['message'] : __( 'The site profile was not accepted.', 'repeatpay-subscriptions-for-woocommerce' );
			return new WP_Error( 'repeatpay_profile_rejected', sanitize_text_field( $message ) );
		}
		if ( ! $consent ) {
			delete_option( 'repeatpay_site_profile_instance_id' );
		}
	...
	private static function profile( $consent ) {
		...
		return array(
			...
			'plugin_name'        => 'RepeatPay Subscriptions for WooCommerce',
			'plugin_slug'        => 'repeatpay-subscriptions-for-woocommerce',
			'plugin_version'     => REPEATPAY_VERSION,
			'product'            => 'repeatpay-subscriptions-for-woocommerce',
	...
	private static function instance_id() {
		$id = get_option( 'repeatpay_site_profile_instance_id' );
		if ( ! $id ) {
			$id = wp_generate_uuid4();
			update_option( 'repeatpay_site_profile_instance_id', $id, false );
		}
	...
	private static function endpoint() {
		return apply_filters( 'repeatpay_site_profile_api_url', self::ENDPOINT );
	}
	...
	public static function privacy_policy_content() {
		...
		wp_add_privacy_policy_content(
			__( 'RepeatPay Subscriptions for WooCommerce', 'repeatpay-subscriptions-for-woocommerce' ),
